...
# יצירת קובץ CSV
with open('HFU_full_articles.csv', 'w', newline='', encoding='utf-8-sig') as f:
    writer = csv.writer(f)
    ...
    # לולאה לפי חודשים: מהחודש הנוכחי עד דצמבר 2012
    today = datetime.today()
    year = today.year
    month = today.month
    ArtNum=0
    while year > 2012 or (year == 2012 and month >= 12):
        for page in range(1, 5):  # עד שלושה עמודים לכל חודש
            if page == 1:
                url = f'https://www.haifa.ac.il/{year}/{month:02d}/'
            else:
                url = f'https://www.haifa.ac.il/{year}/{month:02d}/page/{page}/'
            print(f"🔄 Scraping: {url}")
            try:
                driver.get(url)
                time.sleep(5)
                soup = BeautifulSoup(driver.page_source, 'html.parser')
                articles = soup.select('article.blog-entry')

                print(f"🔍 Found {len(articles)} articles in {month:02d}/{year}")

                for article in articles:
                    try:
                        title_tag = article.select_one('h2.blog-entry-title a')
                        date_tag = article.select_one('li.meta-date')

                        if not title_tag or not title_tag['href']:
                            continue

                        title = title_tag.get_text(strip=True)
                        full_url = title_tag['href'].strip()
                        ...
                        content_tags = inner_soup.select('div.elementor-widget-container')
                        paragraphs = []
                        ...
                        if content:
                            ...
                            writer.writerow(
                                ['HAIFA', date, title, full_url, safe_results[2], safe_results[3], safe_results[4],
                                 safe_results[5], safe_results[6], safe_results[7], article_length])
                            ArtNum+=1
                            print(f"✔️ Saved: {title} ({ArtNum} words)")
                    except Exception as e:
